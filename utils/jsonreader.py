...
class JSONReader(object):
    ...
    def read_json(self, uri, dict_format=True):
        ...
        if dict_format:
            ...
            dfile = '{uri}/chunks/processed/processed0*.json'
            return sql.read.json(dfile)

    def write_json(self, uri):
        try:
            self.data.write.json.save(f'{uri}/dumps/processed.json')
        except:
            # log event
            return False 
        return True
...
class JSONPreprocesser(JSONReader):
    ...
    def transform(self): 
        ...
        self.data_dict['r'] = self.data_dict['r'].join(F.broadcast(most_helpful), 
                                                       on='asin', how='left')
        ...
        self.data = map_to_numeric(self.data, 'asin')
        self.data = map_to_numeric(self.data, 'reviewerID')
        next_reviewer_id = self.data.agg({'numeric_reviewerID':'max'})\
                                    .collect()[0][0] + 1
...
class JSONDumper(JSONReader):
    ...
    def make_data_dict_dumps(cls, bucket_uri):
        ...
        next_reviewer_id = preprocesser.transform()
        np.save(f'{bucket_uri}/dumps/next_reviewer_id.npy', next_reviewer_id)
